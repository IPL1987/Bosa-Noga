...
function Product() {
   ...
   useEffect(() => {
      if (product) 
      setStock(product.sizes.filter( size => size.avalible === true));
   }, [product]);
               ...
               {inStock ? (
                ...
                {product.sizes?.map((size) => (
                  <button key={size.size}
                    className={`catalog-item-size ${size.size === size ? 'selected' : ''} ${size.available ? '' : 'disabled'}`}
                    disabled={!size.available}
                    onClick={() => onChangeSize(size.size)}>{size.size}</button>
                ))}
              </p>
